...
- 정리된 원문이 1,000자 이상이고 분할 가능한 공백·구두점 경계가 9개 이상이면, 원래 장 수와 관계없이 문단·문장 경계를 우선해 정확히 10개 독서 챕터로 균등 분할한다.
- 최소 조건을 만족하지 못한 원문은 10개의 의미 있는 챕터를 만들 수 없으므로 콘텐츠 Job을 `FAILED`로 기록한다.
- 각 독서 챕터에는 장면 1개를 연결하므로 책당 장면 수도 10개다.
- 관리자 표지 생성 API는 별도 스타일 입력 없이 구조화된 한국 웹툰 표지 프롬프트를 항상 적용한다. 세로 `1024x1536` 구도, 썸네일 가독성, 시대 배경, 텍스트·로고 제외 조건을 프롬프트에 명시한다.
...
- 대기 큐는 20개이며, 포화 시 250ms 단위로 용량과 executor 종료 여부를 확인하며 생산 측에 backpressure를 적용하고 작업을 조용히 유실하지 않는다.
